...
class IST_Admin {
	...
	public function page_dashboard(): void {
		...
		global $wpdb;
		$counts = array(
			'tyfcb'     => (int) $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->prefix}ist_tyfcb" ),
			'referrals' => (int) $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->prefix}ist_referrals" ),
			'connects'  => (int) $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->prefix}ist_connects" ),
		);

		ist_get_template( 'admin/tmpl-dashboard.php', compact( 'counts' ) );
	}
}
